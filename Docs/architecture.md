...
- Every compatible Xcode PID in the cached inventory owns an independent route. The runtime starts one primary bridge process for every route as soon as membership is established, while keeping canonical initialize-result publication single-writer. Once the canonical result is published, every remaining route is explicitly initialized and cataloged; workspace and tab ownership then select the matching route.
