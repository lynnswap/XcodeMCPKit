...
Spec一次情報: [Transports](https://modelcontextprotocol.io/specification/2025-06-18/basic/transports)(以下T-§)、[Lifecycle](https://modelcontextprotocol.io/specification/2025-06-18/basic/lifecycle)(L-§)、[Cancellation](https://modelcontextprotocol.io/specification/2025-06-18/basic/utilities/cancellation)。引用は正規化した節名+番号。
...
## [P0・high] Origin 検証が /health と /debug/* endpoints で免除され spec の MUST (all incoming connections) から漏れる (http-gateway)
...
## [medium] POST応答が常に単一JSONのため、進行中request関連のserver→client requestがGET stream/bufferへ迂回し、drop後もproxyがupstreamへfailureを伝播しない (proxy-session)
EVIDENCE: HTTPRequestValidator.swift:49-50 (postPreference常時false、コメントのみでdoc未記載)。server発requestはrouteServerInitiatedPayloads (UpstreamRouting.swift:894-969) → JSONRPCResponseRouter.notify (JSONRPCResponseRouter.swift:267-273) → GET SSE or buffer。bufferは50件超で黙ってremoveFirst (JSONRPCResponseRouter.swift:275-282、ログ無し)。ServerRequestTracker.routeTimeoutの既定300秒は`expiresAt`を付け、record/lookup/consume/complete時にlazy-pruneするroute mapping TTLにすぎない。timer、upstream failure/cancel、response deadlineは実装せず、upstreamの実際の待機時間は未確認。spec T-§Sending 6は関連messageをPOST SSE streamへ載せる想定(SHOULD)、T-§Listening 4はGET stream上のmessageは無関係SHOULD。
...
DIRECTION: 仮説: direct SDKとstdio adapterで共有する単一session-lifecycle authorityをdesign gateで定める。`Mcp-Session-Id`付き要求の404だけをtyped `.sessionExpired`とし、旧session/transport identityをキーにsingle-flightでfresh transport + session IDなしinitialize + `notifications/initialized`を行う。spec MUSTは新sessionのinitializeまで。元操作をreplayするproduct contractを採るなら最大1回、2回目の404はsurfaceし、初回attemptからの1 logical deadlineを共有する。delivery不明のside-effecting requestは自動再送しない。SSE GET 404もsilent terminalにしない。
