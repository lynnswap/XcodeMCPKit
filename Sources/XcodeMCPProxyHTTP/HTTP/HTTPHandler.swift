...
final class HTTPHandler: ChannelInboundHandler, Sendable {
    ...
    let config: ProxyHTTPConfiguration
    ...
    init(
        ...
        controlService: HTTPControlService,
        scheduleResponseCompletion: @escaping @Sendable (
            EventLoop,
            @escaping @Sendable () -> Void
        ) -> Void = { eventLoop, operation in
        ...
        self.scheduleResponseCompletion = scheduleResponseCompletion
    }
    ...
    private func handleDelete(context: ChannelHandlerContext, head: HTTPRequestHead, requestLog: RequestLogContext) {
        ...
        controlService.deleteSession(ProxySessionID(rawValue: sessionID))
    ...
    private func validateSession(
        ...
        let sessionIDValue = ProxySessionID(rawValue: sessionID)
        let sessionState = controlService.sessionState(sessionIDValue)
        guard sessionState != .missing else {
            _ = sendPlain(
                on: channel,
                status: .notFound,
        ...
        guard case .initialized(let negotiatedVersion) = sessionState else {
            _ = sendPlain(
                on: channel,
                status: .badRequest,
