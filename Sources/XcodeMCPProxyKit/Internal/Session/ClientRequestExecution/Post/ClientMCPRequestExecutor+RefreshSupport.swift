...
extension ClientMCPRequestExecutor {
    ...
    func forwardOnce(
        ...
        case .forward(let index):
            preferredUpstreamIndices = index.map { [$0] }
        case .forwardAny(let indices):
        ...
        do {
            ...
            let resolution = try await sessionManager.enqueueOnUpstreamSlot(
                ...
                guard let attemptRequestObject = try? JSONRPC.Wire.object(fromData: bodyData) else {
    ...
    func forwardRefreshCodeIssuesRequest(
        ...
        await refreshWorkflow.run(
            ...
            internalUpstreamChooser: { _ in self.sessionManager.chooseUpstreamIndex() },
            internalToolCaller: { name, arguments, sessionID, eventLoop, upstreamIndex, timeout in
