...
package struct ExistingProxyServerClient: DependencyClient {
    ...
    package var detectExistingProxyServerPIDs: @Sendable (_ host: String, _ port: Int) -> [Int]
    ...
    package init(
        ...
        detectExistingProxyServerPIDs: @escaping @Sendable (_ host: String, _ port: Int) -> [Int]
    ) {
    ...
    package static func live(
        ...
        Self(
            ...
            detectExistingProxyServerPIDs: { host, port in
                detectExistingProxyServerPIDs(
                    host: host,
                    port: port,
                    discoveryClient: discoveryClient,
                    runCommand: runCommand
    ...
    private static func terminateExistingProxyServerIfNeeded(
        ...
        let pids = listeningPIDs(onTCPPort: port, matchingHost: host, runCommand: runCommand)
        guard !pids.isEmpty else { return false }

        let currentPID = currentProcessID()
        var didTerminate = false
        for pid in pids where pid != currentPID {
            guard isProxyServerProcess(pid: pid, runCommand: runCommand) else { continue }
            emitWarning(terminationWarning(port: port, pid: pid))
            if terminate(pid: pid, clock: clock, sendSignal: sendSignal) {
                didTerminate = true
            }
        }
        if didTerminate {
    ...
    private static func detectExistingProxyServerPIDs(
        host: String,
        port: Int,
        discoveryClient: DiscoveryClient,
        runCommand: @escaping @Sendable (_ launchPath: String, _ arguments: [String]) -> String?
        ...
        for pid in listeningPIDs(onTCPPort: port, matchingHost: host, runCommand: runCommand)
        where isProxyServerProcess(pid: pid, runCommand: runCommand) {
            pids.append(pid)
        }
    ...
    private static func listeningPIDs(
        onTCPPort port: Int,
    ...
    private static func waitForPortToBeFree(
        ...
        let deadline = clock.now().addingTimeInterval(timeout)
        while clock.now() < deadline {
            if listeningPIDs(onTCPPort: port, matchingHost: host, runCommand: runCommand).isEmpty {
                return
            }
            clock.sleepForTimeInterval(0.05)
        }
