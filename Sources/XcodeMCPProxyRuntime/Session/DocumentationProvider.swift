import XcodeMCPDocumentationSearch
import Foundation
import Logging
import NIO
import XcodeMCPCore
...
struct LiveDocumentationSearchServiceRepairer: DocumentationSearchServiceRepairing {
    private let repairer: InstalledDocumentationSearchRepairer
    ...
    init(
        ...
        repairer = InstalledDocumentationSearchRepairer(assetRoot: assetRoot, currentOSVersion: currentOSVersion,
                                                       readConfigURLOverride: readConfigURLOverride,
                                                       writeConfigURLOverride: writeConfigURLOverride)
    }

    func repairDocumentationSearch(for target: XcodeProcessTarget) async -> DocumentationSearchServiceRepairResult {
        await repairer.repair(for: target.documentationSearchInstallation)
    }
}

extension XcodeProcessTarget {
    var documentationSearchInstallation: DocumentationSearchInstallation {
        DocumentationSearchInstallation(appPath: appPath, xcodeVersion: xcodeVersion)
    }
}
    ...
    enum ToolCatalog {
        ...
        static func errorTextIsDocumentationProviderFailure(_ text: String) -> Bool {
            ...
            return normalized.contains("config.json")
                ...
                || documentationSearchTextEncoderInitializationFailed(normalized)
        }
...
struct DocumentationSearchActionProvider: DocumentationSearchProviding {
    ...
    private let backend: InstalledDocumentationSearch
    ...
    init(
        ...
        defaultTargetResolver: @escaping @Sendable () -> XcodeProcessTarget? = Self.defaultXcodeProcessTarget,
        currentOSVersion: @escaping @Sendable () -> String = DocumentationSearchAssetLocator.currentOperatingSystemVersionString
    ) {
        backend = InstalledDocumentationSearch(assetRoot: assetRoot, invoker: invoker, currentOSVersion: currentOSVersion)
        self.defaultTargetResolver = defaultTargetResolver
    }

    func descriptor(for target: XcodeProcessTarget) async -> JSONValue? {
        guard let resolvedTarget = resolveTarget(target),
              await backend.isAvailable(for: resolvedTarget.documentationSearchInstallation) else { return nil }
        return Self.descriptor
    }

    func callDocumentationSearch(requestData: Data, for target: XcodeProcessTarget, timeout: TimeAmount?) async throws -> Data {
        guard timeout.map({ $0.nanoseconds > 0 }) ?? true else { throw TimeoutError() }
        let arguments = try Self.searchArguments(from: requestData)
        guard let resolvedTarget = resolveTarget(target) else { throw UpstreamSlotScheduler.AcquisitionError.unavailable }
        do {
            let output = try await backend.search(arguments.query, frameworks: arguments.frameworks, limit: arguments.limit,
                                                 in: resolvedTarget.documentationSearchInstallation, timeout: timeout)
            return try Self.makeResponse(requestID: arguments.requestID, output: output)
        } catch DocumentationSearchBackendError.unavailable {
            throw UpstreamSlotScheduler.AcquisitionError.unavailable
        } catch DocumentationSearchBackendError.invalidResponse(let reason) {
            throw ControlPlane.Error.invalidResponse(reason)
        }
    }
